...
app.config['SQLALCHEMY_DATABASE_URI'] = database_url or 'postgresql://localhost/bput_local'
...
def seed_database():
    """Seed the database with data from CSV if tables are empty"""
    try:
        ...
        # Read CSV
        csv_path = os.path.join(os.path.dirname(__file__), 'internship_posted_data.csv')
    ...
    if request.method == 'POST':
        ...
        if 'profile_photo' in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                photo_url = upload_to_cloudinary(file, folder="student_profiles")
                if photo_url:
...
def add_certificate():
    ...
    if allowed_file(file.filename):
        cert_url = upload_to_cloudinary(file, folder="certificates")
        if cert_url:
            ...
            db.session.add(new_certificate)  # ✅ Move inside the if block
            db.session.commit()               # ✅ Move inside the if block
...
def company_edit_profile():
    ...
    if request.method == 'POST':
        ...
        if 'logo' in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                logo_url = upload_to_cloudinary(file, folder="company_logos")
                if logo_url:
...
if __name__ == '__main__':
    ...
    # This runs when deployed (gunicorn imports the app)
    with app.app_context():
        if not os.path.exists(UPLOAD_FOLDER):
            os.makedirs(UPLOAD_FOLDER)
        db.create_all()
        seed_database()             # i am so tired , for to present on presentation , i am going to sleep